...
private const val USER_REFERENCE = "teste"
        ...
        disposables.add(
            Observable
                .create<Bundle> { emitter ->
                    ...
                    logger.debug("initBTConnection -> $initBtConnection")
                ...
                .create<Bundle> { emitter ->
                    ...
                    val isQrCode = transaction.method == PlugPag.TYPE_QRCODE_ELO_CREDITO ||
                        transaction.method == PlugPag.TYPE_QRCODE_ELO_DEBITO ||
                            transaction.method == PlugPag.TYPE_PIX

                    if(transaction.method == PlugPag.TYPE_PIX) {
                        emitter.onNext(
                            Bundle().apply {
                                putInt(PaymentContract.PP_CODE, 1049)
                                putString(PaymentContract.PP_MESSAGE, "Funcionalidade não permitida")
                            }
                        )
                        emitter.onComplete()
                    }
                    ...
                    val transactionMethod =
                        PlugPagVoidData.TypeCardTransaction.getBy(transaction.method)
            ...
            disposables.add(
                Observable
                    ...
                    .subscribeBy(
                        ...
                        onComplete = {
                            logger.debug("Send sms", "Finished")
                        },
                        onError = {
                            logger.debug("Finished send sms", "Error")
                        }
                    )
            )
        }
